...
每個角色接受 `model`（`opus` / `sonnet` / `haiku`）與 `effort`（`low` /
`medium` / `high` / `xhigh` / `max`）。Effort 會以 prompt-level keyword
（`Think.`、`Think hard.`、`Think harder.`、`Ultrathink.`）注入每個
subagent prompt 開頭——這是因應 Claude Code 的 `Agent` 工具目前在呼叫時
還不接受 `effort` 參數所做的橋接。等 Anthropic 加上原生 effort 後，
schema 不必改、`/sprint` 會自動切過去。
...
| v2.3.0 | Claude 端加入每角色 reasoning effort（low/medium/high/xhigh/max），schema v4 | 目前版本 |
